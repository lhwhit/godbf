...
func (dt *DbfTable) MyAddTextField(fieldName string, length byte) (err error) {
	return dt.MyaddField(fieldName, Character, length, Character.decimalCountNotApplicable())
}
...
func (dt *DbfTable) MyaddField(fieldName string, fieldType DbaseDataType, length byte, decimalPlaces uint8) (err error) {

	if dt.dataEntryStarted {
		return errors.New("Once you start entering data to the dbase table or open an existing dbase file, altering dbase table schema is not allowed!")
	}

	normalizedFieldName := dt.MynormaliseFieldName(fieldName)
	...
	slice := dt.MyconvertToByteSlice(df.name, 10)
...
func (dt *DbfTable) MynormaliseFieldName(name string) (s string) {
	s = name
...
func (dt *DbfTable) MyconvertToByteSlice(value string, numberOfBytes int) (s []byte) {

	by := []byte(value)

	if len(value) > numberOfBytes {
		s = by[0:numberOfBytes]
	} else {
		s = by
	}
	return
...
// My change GBK 汉字直接写，不需要转换字符集。
// SetFieldValue sets the value for the given row and field index as specified
// If the field index is invalid, or the value is incompatible with the field's type, an error is returned.
func (dt *DbfTable) SetFieldString(row int, fieldIndex int, value string) (err error) {
